...
3\. Create the lab in the mentioned regions for availability of AWS resources.
...
1\. Go to the EC2 Dashboard in the Management Console and click on **Instances** in the left menu.  Select the EC2 instance created.  
...
<pre>$ git clone http://infygit.ad.infosys.com/Sthiti_Pragnya/awscapestoneproject2.git</pre>

3\. Navigate to the lab-2-build/mxnet/ folder to use as your working directory.  
<pre>$ cd awscapestoneproject2/lab-2-build/mxnet</pre>
...
Choose your ECS Cluster from the dropdown menu. You can leave all other fields as default.  Keep number of tasks set to 1 and click **Run Task**.  
...
The command references an OUTPUTBUCKET environment variable, and you can set this in **Env variables**.  Set the key to be "OUTPUTBUCKET" and the value to be the S3 output bucket created by you. Set "AWS_DEFAULT_REGION" to be the value of the region.

![Advanced Configuration - Environment](/images/adv-config-env-train.png)  

Next you'll configure logging to CloudWatch Logs.  Scroll down to the **Log configuration**, select **awslogs** from the *Log driver* dropdown menu.  For *Log options*, set the **awslogs-group** to be the value of **cloudWatchLogsGroupName** .  And type in the region you're currently using, e.g. Ohio would be us-east-2, Oregon would be us-west-2.  Leave the **awslogs-stream-prefix** blank.  
